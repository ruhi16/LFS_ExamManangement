...
class ExamTypeComp extends Component
{
    ...
    {
        $this->isDataFinalized = Exam02Type::where('is_finalized', true)->exists();
    }
    ...
    public function finalizeData()
    {
        if ($this->finalizingId) {
            $examType = Exam02Type::findOrFail($this->finalizingId);
            $examType->update(['is_finalized' => true]);
            
            $this->checkGlobalFinalizationStatus();
            session()->flash('message', 'Exam type finalized successfully! No further changes allowed.');
        }
    ...
    {
        $examType = Exam02Type::findOrFail($id);
        $examType->update(['is_finalized' => false]);
        
        $this->checkGlobalFinalizationStatus();
        session()->flash('message', 'Exam type unfinalized successfully! Changes are now allowed.');
    }
