                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center">
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $examName->name }}</div>
                                    @if($examName->is_finalized)
                        ...
                        <td class="px-4 py-3">
                            <div class="text-sm text-gray-900">{{ $examName->description ?: '-' }}</div>
                            @if($examName->remarks)
                            <div class="text-xs text-gray-500">{{ Str::limit($examName->remarks, 50) }}</div>
                            @endif
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                            {{ $examName->order_index ?: '-' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium 
                                                                            @if($examName->is_active) bg-green-100 text-green-800 @else bg-red-100 text-red-800 @endif">
                                    {{ $examName->is_active ? 'Active' : 'Inactive' }}
                                </span>
                                @if($examName->status)
                                <span class="text-xs text-gray-500">{{ $examName->status }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                @if(!$isDataFinalized)
                                <button wire:click="toggleStatus({{ $examName->id }})"
                                    class="text-indigo-600 hover:text-indigo-900 text-xs">
                                    {{ $examName->is_active ? 'Deactivate' : 'Activate' }}
                                </button>
                                <button wire:click="edit({{ $examName->id }})"
                                    class="text-indigo-600 hover:text-indigo-900 text-xs">
                                    Edit
                                </button>
                                <button wire:click="confirmDelete({{ $examName->id }})"
                                ...
                                @if($examName->is_finalized)
                                <button wire:click="unfinalizeData({{ $examName->id }})"
                                ...
                                @else
                                <button wire:click="confirmFinalize({{ $examName->id }})"
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900" id="modal-title">
                                {{ $editingId ? 'Edit Exam Name' : 'Add New Exam Name' }}
                            </h3>
                        </div>
