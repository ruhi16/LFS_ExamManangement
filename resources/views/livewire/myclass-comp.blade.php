<div class="flex-1 p-6 overflow-y-auto max-w-6xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Class Management</h1>
                <p class="mt-1 text-sm text-gray-600">Manage school classes and their configurations</p>
            </div>
        ...
        <div class="flex items-center space-x-4">
            <div class="flex-1">
                ...
                <input wire:model.debounce.300ms="search" type="text" id="search"
                    placeholder="Search classes by name, description, or status..."
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($myclasses as $myclass)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $myclass->name }}</div>
                                        @if($myclass->is_finalized)
                            ...
                            <td class="px-4 py-3">
                                <div class="text-sm text-gray-900">{{ $myclass->description ?: '-' }}</div>
                                @if($myclass->remarks)
                                    <div class="text-xs text-gray-500">{{ Str::limit($myclass->remarks, 50) }}</div>
                                @endif
                            </td>
                                ...
                                <div class="flex items-center space-x-2">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium 
                                                                            @if($myclass->is_active) bg-green-100 text-green-800 @else bg-red-100 text-red-800 @endif">
                                        {{ $myclass->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                    @if($myclass->status)
                                        <span class="text-xs text-gray-500">{{ $myclass->status }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <button wire:click="toggleStatus({{ $myclass->id }})"
                                        class="text-indigo-600 hover:text-indigo-900 text-xs">
                                        {{ $myclass->is_active ? 'Deactivate' : 'Activate' }}
                                    </button>
                                    <button wire:click="edit({{ $myclass->id }})"
                                        class="text-indigo-600 hover:text-indigo-900 text-xs">
                                        Edit
                                    </button>
                                    <button wire:click="confirmDelete({{ $myclass->id }})"
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700 leading-5">
                                Showing
                                <span class="font-medium">{{ $myclasses->firstItem() }}</span>
                                to
                                <span class="font-medium">{{ $myclasses->lastItem() }}</span>
                                of
                                <span class="font-medium">{{ $myclasses->total() }}</span>
                                results
                            </p>
                        </div>
                        ...
                        <div>
                            <span class="relative z-0 inline-flex shadow-sm rounded-md">
                                ...
                                {{-- Pagination Elements --}}
                                @foreach ($myclasses->getUrlRange(1, $myclasses->lastPage()) as $page => $url)
                                    @if ($page == $myclasses->currentPage())
