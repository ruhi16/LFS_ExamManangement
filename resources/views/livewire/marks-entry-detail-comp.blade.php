<div class="flex-1 p-6 overflow-y-auto max-w-full mx-auto">
    ...
    @if($examClassSubject && $examClassSubject->is_finalized)
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" wire:model.lazy="marks.{{ $student->id }}"
                                        class="w-20 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-center
                                                        @if($examClassSubject && $examClassSubject->is_finalized) bg-gray-100 cursor-not-allowed @endif" placeholder="0" min="0"
                                        max="100" step="0.01" @if($examClassSubject && $examClassSubject->is_finalized)
                                    readonly @endif>
                                    @endif
                                    ...
                                    <div class="absolute -top-1 -right-1">
                                        <input type="checkbox" wire:model="absentStudents.{{ $student->id }}"
                                            class="w-4 h-4 text-red-600 bg-white border-2 border-red-300 rounded focus:ring-red-500 focus:ring-2
                                                        @if($examClassSubject && $examClassSubject->is_finalized) cursor-not-allowed @endif" title="Mark as Absent" @if($examClassSubject
                                            && $examClassSubject->is_finalized) disabled @endif>
                                    </div>
                                </div>
            ...
            <div class="flex justify-between items-center">
                <div class="flex space-x-2">
                    ...
                    <button wire:click="saveMarks"
                        ...
                        @if($examClassSubject && $examClassSubject->is_finalized) disabled @endif>
                        Save Marks
                    </button>
                </div>
                ...
                <div class="flex space-x-2">
                    @if($examClassSubject)
                    @if($examClassSubject->is_finalized)
