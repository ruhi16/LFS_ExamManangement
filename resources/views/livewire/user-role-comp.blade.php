<div class="p-6">
    ...
    <div class="mb-4 p-3 bg-yellow-100 border border-yellow-400 rounded text-sm">
        <strong>🐛 DEBUG:</strong>
        showRoleModal = <span class="font-bold {{ $showRoleModal ? 'text-green-600' : 'text-red-600' }}">{{
            $showRoleModal ? 'TRUE' : 'FALSE' }}</span>,
        showUserModal = <span class="font-bold {{ $showUserModal ? 'text-green-600' : 'text-red-600' }}">{{
            $showUserModal ? 'TRUE' : 'FALSE' }}</span>,
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            ...
            <button wire:click="cleanupTeacherAssignments"
                onclick="return confirm('This will clean up inconsistent teacher-user assignments. Continue?')"
                class="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors">
                <i class="fas fa-tools mr-2"></i>Cleanup DB
            </button>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        {{-- <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                @if($currentUserRole >= 4 && $role->id < $currentUserRole) <button
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex flex-col space-y-2">
                                    <div class="flex space-x-2 items-center">
                                        @if($user->role_id < $currentUserRole) <!-- Edit User -->
                                            ...
                                            <div class="relative inline-block">
                                                <select
                                                    ...
                                                    <option value="{{ $availableRole->id }}">{{ $availableRole->name }}
                                                    </option>
                                                    @endif
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="relative inline-block">
                                                <select onchange="
                                                                    console.log('Teacher assignment triggered');
                                                                    console.log('Selected value:', this.value);
                                                                    console.log('User ID:', {{ $user->id }});
                                                                    if(this.value && this.value !== '') { 
                                                                        console.log('Calling assignTeacher with:', {{ $user->id }}, 'and', this.value);
                                                                        @this.call('assignTeacher', {{ $user->id }}, parseInt(this.value)); 
                                                                        this.value=''; 
                                                                    } else {
                                                                        console.log('No value selected, skipping assignment');
                                                                    }
                                                                "
                                                    ...
                                                    <option value="{{ $t->id }}">{{ $t->name }} (ID: {{ $t->id }})
                                                    </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <div class="text-xs text-gray-600">
                                        @if((int)($user->teacher_id ?? 0) > 0)
                                        Assigned Teacher: <span class="font-semibold">{{ optional($user->teacher)->name
                                            }} (ID: {{ $user->teacher_id }})</span>
                                        @else
                                        No Teacher Assigned
                                        @endif
                                    </div>
