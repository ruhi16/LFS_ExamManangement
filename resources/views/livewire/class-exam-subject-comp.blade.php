<div class="flex-1 p-6 overflow-y-auto max-w-full mx-auto">
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Configure which subjects are available for each class, exam, and exam type combination</p>
            </div>
    ...
    @if($selectedClassId && count($classSubjects) > 0 && count($examNames) > 0)
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="p-4 border-b border-gray-200 bg-gray-50">
                <h3 class="text-lg font-semibold text-gray-900">Subject-Exam Configuration Matrix</h3>
                <p class="text-sm text-gray-600 mt-1">
                    Select which subjects are available for each exam and exam type combination
                </p>
            </div>
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            @foreach($examNames as $examName)
                                <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-l border-gray-300">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                @foreach($examNames as $examName)
                                    <td class="px-4 py-4 text-center border-l border-gray-300">
                                        <div class="grid grid-cols-{{ count($examTypes) }} gap-2">
                                            @foreach($examTypes as $examType)
                                                @php
                                                    $examDetailKey = $examName->id . '_' . $examType->id;
                                                    $hasExamDetail = isset($examDetails[$examDetailKey]);
                                                    $isSelected = $subjectSelections[$subject->subject_id][$examName->id][$examType->id] ?? false;
                                                @endphp
                                                ...
                                                <div class="flex flex-col items-center">
                                                    @if($hasExamDetail)
                                                        ...
                                                        @endif
                                                    @else
                                                        <div class="h-4 w-4 bg-gray-200 rounded border-2 border-gray-300" title="No exam detail configured"></div>
    ...
    @elseif($selectedClassId && count($classSubjects) === 0)
