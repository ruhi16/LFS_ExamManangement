<div class="flex-1 p-6 overflow-y-auto max-w-6xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Exam Type Management</h1>
                <p class="mt-1 text-sm text-gray-600">Manage exam types and their configurations</p>
            </div>
        ...
        <div class="flex items-center space-x-4">
            <div class="flex-1">
                ...
                <input wire:model.debounce.300ms="search" type="text" id="search"
                    placeholder="Search exam types by name, description, or status..."
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @if($examType->remarks)
                            <div class="text-xs text-gray-500">{{ Str::limit($examType->remarks, 50) }}</div>
                            @endif
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                            {{ $examType->order_index ?: '-' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium 
                                                                            @if($examType->is_active) bg-green-100 text-green-800 @else bg-red-100 text-red-800 @endif">
                                    {{ $examType->is_active ? 'Active' : 'Inactive' }}
                                </span>
                                @if($examType->status)
                                <span class="text-xs text-gray-500">{{ $examType->status }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                @if($examType->is_finalized)
                                <button wire:click="unfinalizeData({{ $examType->id }})"
                                    onclick="return confirm('Are you sure you want to unfinalize this exam type? This will allow changes again.')"
                                    class="bg-orange-500 hover:bg-orange-700 text-white px-2 py-1 rounded text-xs">
                                    🔓 Unfinalize
                                </button>
                                @else
                                <button wire:click="confirmFinalize({{ $examType->id }})"
                                    class="bg-green-500 hover:bg-green-700 text-white px-2 py-1 rounded text-xs">
                                    🔒 Finalize
                                </button>
                                <button wire:click="toggleStatus({{ $examType->id }})"
                                    class="text-indigo-600 hover:text-indigo-900 text-xs">
                                    {{ $examType->is_active ? 'Deactivate' : 'Activate' }}
                                </button>
                                <button wire:click="edit({{ $examType->id }})"
                                    class="text-indigo-600 hover:text-indigo-900 text-xs">
                                    Edit
                                </button>
                                <button wire:click="confirmDelete({{ $examType->id }})"
                    ...
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">
                            <div class="text-lg font-medium mb-2">No exam types found</div>
                            <div class="text-sm">
                                @if($search)
                                No exam types match your search criteria.
                                @else
                ...
                <form wire:submit.prevent="save">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            <!-- Name -->
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700">Exam Type *</label>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Delete Exam Type</h3>
                            <div class="mt-2">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Finalize Exam Type</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to finalize this exam type? Once finalized, it cannot be
                                    edited or deleted.
                                </p>
                            </div>
                        </div>
